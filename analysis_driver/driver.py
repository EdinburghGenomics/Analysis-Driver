...
def _output_data(sample_dir, sample_id, output_fileset):
    ...
    if output_fileset == 'bcbio':
        ...
        # intra-species contamination check
        ...
        self_sm = util.find_file(sample_dir, user_sample_id + '.selfSM')
        self_sm_link = os.path.join(dir_with_linked_files, os.path.basename(self_sm))
        if self_sm:
            if not os.path.islink(self_sm_link):
                os.symlink(self_sm, self_sm_link)
        else:
