...
def setup_bcbio_run(template, csv_file, job_dir, fastqs):
    """
    Call bcbio -w template on relevant input files.
...
def _localexecute(*args):
    ...
    executor.start()
    exit_status = executor.join()
    app_logger.info('Exit status: ' + str(exit_status))
    return exit_status
