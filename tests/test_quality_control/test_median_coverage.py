...
class TestSamtoolsDepth(QCTester):
    ...
    def test_get_samtools_depth_command(self):
        ...
        assert my_samtools_depth_command == """path/to/samtools depth -a -a -q 0 -Q 0 testfile.bam | awk -F "\t" '{array[$1"\t"$3]+=1} END{for (val in array){print val"\t"array[val]}}' | sort -k 1,1 -nk 2,2 > test_sample.depth"""

        assert my_samtools_depth_outfile == 'test_sample.depth'
    ...
    def test_run_samtools_depth(self, mocked_execute):
        ...
        mocked_execute.assert_called_once_with(["""path/to/samtools depth -a -a -q 0 -Q 0 testfile.bam | awk -F "\t" '{array[$1"\t"$3]+=1} END{for (val in array){print val"\t"array[val]}}' | sort -k 1,1 -nk 2,2 > test_sample.depth"""],
