...
class TestWellDuplicates(QCTester):
    ...
    def test__well_duplicates(self, mocked_execute):
        welldup = WellDuplicates(self.run_dataset, working_dir='test_run', run_directory='path/to/run')
        instance = mocked_execute.return_value
        instance.join.return_value = 0
        exit_status = welldup._well_duplicates()
        assert exit_status == 0
        mocked_execute.assert_called_once_with(
                ['path/to/well_duplicate -f path/to/coord_file -r path/to/run -s hiseq_x > test_run/test_run.wellduplicate 2> test_run/test_run.wellduplicate.err'],
                cpus=1,
                job_name='welldup',
                mem=2,
                log_commands=False,
